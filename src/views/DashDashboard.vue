<template>
  <div class="min-h-screen bg-base-200 p-4 md:p-8">
    <div class="max-w-7xl mx-auto">
      ...
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-4xl font-bold text-primary mb-2">DASH.js Player Metrics Dashboard</h1>
        <p class="text-base-content/70">Real-time streaming metrics visualization</p>
      </div>
        ...
        <div class="card-body">
          ...
          <div class="flex flex-wrap gap-2 mt-4">
            ...
            >
              ...
              <option v-for="quality in availableQualities" :key="quality.index" :value="quality.index">
...
// Icons
const icons = {
  bitrate: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z',
  buffer: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15',
  frames: 'M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z',
...
const updateThroughputMetric = (e) => {
  ...
  if (request && request.type === 'MediaSegment') {
    const speed = ((request.bytesLoaded * 8) / (request.requestEndDate.getTime() - request.requestStartDate.getTime()) / 1000).toFixed(2)
...
const updateCharts = () => {
  ...
  charts.latency.data.datasets[0].data = metricsHistory.latency.length > 0
...
const updateAverageAndPeak = () => {
  ...
  if (metricsHistory.bitrate.length > 0) {
    const avgBitrate = Math.round(metricsHistory.bitrate.reduce((a, b) => a + b, 0) / metricsHistory.bitrate.length)
  ...
  if (metricsHistory.buffer.length > 0) {
    const avgBuffer = (metricsHistory.buffer.reduce((a, b) => a + b, 0) / metricsHistory.buffer.length).toFixed(2)
  ...
  if (metricsHistory.throughput.length > 0) {
    const avgSpeed = (metricsHistory.throughput.reduce((a, b) => a + b, 0) / metricsHistory.throughput.length).toFixed(2)
