      ...
      <div class="player-wrap">
        ...
        <div class="representation-controls" v-if="representations.length">
          ...
          <select v-model.number="selectedQualityIndex" @change="setManualQuality">
            ...
            <option v-for="(r, idx) in representations" :key="idx" :value="idx">{{idx}} — {{r.height}}p @ {{formatBitrate(r.bitrate)}}</option>
          </select>
        </div>
      </div>
      ...
      <div class="analysis">
        ...
        <section>
          ...
          <table v-if="representations.length">
            <thead><tr><th>#</th><th>Bandwidth</th><th>Width</th><th>Height</th><th>Codec</th></tr></thead>
            <tbody>
        ...
        <section>
          ...
          <div class="log" ref="logBox">
            <div v-for="(l, idx) in logs" :key="idx" class="log-line">[{{l.time}}] <strong>{{l.event}}</strong> — {{l.summary}}</div>
          </div>
        </section>
        ...
        <section>
          ...
          <ul>
            <li>dash.js — reference MPEG-DASH web client (use for standard compliance & event debugging)</li>
            ...
            <li>Network inspector (browser DevTools) — HAR capture to analyze segment timing and sizes</li>
...
<script setup>
// Vue 3 single-file component for DASH playback + basic analysis using dash.js
// Usage notes (keep with this file):
// - Install dash.js via npm/yarn: `npm i dashjs` OR include it from CDN in your index.html
// - This SFC uses the dash.js global import. If your bundler resolves `dashjs` module, adjust import accordingly.
...
function loadManifest(){
  ...
  if (playerRef.value){
    try{ playerRef.value.reset() }catch(e){}
  ...
  player.updateSettings({ 'streaming': { 'fastSwitchEnabled': true, 'buffer': { 'stableBufferTime': 20 }}})
  ...
  const handlers = {
    ...
    [E.BUFFER_LEVEL_UPDATED]: (e) => { bufferLevel.value = e?.bufferLevel ?? bufferLevel.value; pushLog('BUFFER_LEVEL_UPDATED', `level=${(bufferLevel.value||0).toFixed(2)}s`) },
    ...
    [E.QUALITY_CHANGE_RENDERED]: (e) => { currentQualityIndex.value = e?.newQuality; pushLog('QUALITY_RENDERED', `new=${e?.newQuality} old=${e?.oldQuality}`) },
    [E.FRAGMENT_LOADING_STARTED]: (e) => { pushLog('FRAGMENT_LOADING_STARTED', e?.request?.url || JSON.stringify(e?.request?._url || {})) },
    [E.FRAGMENT_LOADING_COMPLETED]: (e) => { pushLog('FRAGMENT_LOADING_COMPLETED', e?.request?.url || '') },
    [E.ERROR]: (e) => { pushLog('ERROR', JSON.stringify(e)) }
  }
  ...
  setTimeout(() => {
    try{
      ...
      mpdInfo.periodCount = (player.getDashManifest && player.getDashManifest()?.Period?.length) || 1
      ...
      if (list && list.length){
        representations.value = list.map(item => ({
          ...
          width: item.width, height: item.height, codecs: item.codecs || item.mimeType
        }))
      }
...
onBeforeUnmount(() => {
  try{ playerRef.value && playerRef.value.reset() }catch(e){}
})

const bufferLevelString = computed(() => (bufferLevel.value != null ? bufferLevel.value.toFixed(2) + 's' : '-'))
</script>

<style scoped>
.dash-analyzer{ font-family: system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial; }
.controls{ display:flex; gap:.5rem; align-items:center; padding: .5rem }
.controls input{ flex:1; padding:.4rem }
.main{ display:flex; gap:1rem; padding: .5rem }
.player-wrap{ flex:1; max-width:60% }
.player-wrap video{ width:100%; height:auto; background:#000 }
.quick-stats{ display:flex; gap:1rem; margin-top:.4rem }
.analysis{ flex:1; overflow:auto; max-height:70vh }
.analysis section{ margin-bottom:1rem; background:#fafafa; padding:.6rem; border-radius:6px }
.log{ max-height:200px; overflow:auto; background:#111; color:#eee; padding:.5rem; font-family:monospace }
.log-line{ padding:.15rem 0 }
.representation-controls{ margin-top:.5rem }
table{ width:100%; border-collapse:collapse }
td, th{ padding:.3rem; border-bottom:1px solid #eee; text-align:left }
</style>
